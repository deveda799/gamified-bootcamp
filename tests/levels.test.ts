...
test("calculates Lv1 觉醒者 when total points are zero", () => {
  const result = calculateCurrentLevel(0);

  assert.equal(result.levelNo, 1);
  assert.equal(result.levelName, "觉醒者");
...
test("calculates Lv7 超级个体 when total points reach the final threshold", () => {
  const result = calculateCurrentLevel(1500);

  assert.equal(result.levelNo, 7);
