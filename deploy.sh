...
readonly REPO_URL="https://github.com/deveda799/gamified-bootcamp.git"
...
if [[ -d "${APP_DIR}/.git" ]]; then
  ...
  run_as_deploy git -C "${APP_DIR}" pull --ff-only origin main
else
  run_as_deploy git clone --branch main --single-branch "${REPO_URL}" "${APP_DIR}"
fi
